...
class BlumBot:
    ...
    async def friend_claim(self):
        ...
        if float(r.get('amountForClaim')) and r.get('canClaim'):
            ...
            logger.success(f"Thread {self.thread} | Claim friends reward: {claim_balance}")
    ...
    async def tasks(self):
        for task in await self.get_tasks():
            ...
            if await self.claim_task(task):
                logger.success(f"Thread {self.thread} | Completed task «{task['title']}»")
            else:
                logger.error(f"Thread {self.thread} | Failed complete task «{task['title']}»")
    ...
    async def login(self):
        ...
        if query is None:
            logger.error(f"Thread {self.thread} | Session {self.account}.session invalid")
            await self.logout()
            return None
